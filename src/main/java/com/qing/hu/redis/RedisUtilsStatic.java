 ...
 * @create: 2020/6/30
 */
public class RedisUtilsStatic {
    /**
     * 日志对象
     */
    private static Logger logger = LoggerFactory.getLogger(RedisUtilsStatic.class);
   ...
   private static RedisTemplate redisTemplate =SpringUtil.getBean("redisTemplate",RedisTemplate.class);
     ...
     * @return
     */
    public static  boolean lSetObject(String key, Object value) {
        try {
     ...
     * @return
     */
    public static  boolean lSetObject(String key, Object value, long time) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (time > 0) {
                expire(key, time);
